...
{
    if (argc < 3 || argc > 4) {
        cerr << endl << "Usage: ./stereo_inertial_realsense_t265 path_to_vocabulary path_to_settings (trajectory_file_name)" << endl;
        return 1;
    }
    ...
    while (!SLAM.isShutDown()) {
        ...
        for (int i = 0; i < vGyro.size(); ++i) {
            ...
            if (isnan(vAccel[i].x) || isnan(vAccel[i].y) || isnan(vAccel[i].z) || isnan(vGyro[i].x) || isnan(vGyro[i].y) || isnan(vGyro[i].z) || isnan(vGyro_times[i])) {
                exit(-1);
            }
        }
