...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft, const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    Eigen::Quaterniond deg90(0.7071068, 0, 0.7071068, 0);
    ...
    if (do_rectify) {
        ...
        Sophus::SE3f pos = mpSLAM->TrackStereo(cv_ptrLeft->image, cv_ptrRight->image, cv_ptrLeft->header.stamp.toSec());

        int mp = mpSLAM->GetMatchesPointsNum();
        cout << "orb num: " << mp << endl;
        // cout << "pos: " << pos.matrix() << endl;

        geometry_msgs::TransformStamped transformStamped;

        transformStamped.header.stamp = ros::Time::now();
        transformStamped.header.frame_id = "origin/camera";
        transformStamped.child_frame_id = "orb/camera";
        Eigen::Quaterniond q(pos.so3().matrix().cast<double>());
        // q = q * deg90;
